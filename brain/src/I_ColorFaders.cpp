...
void setCurrentColor() {
  const render::Hsv dialed = render::colorFrom(destinations::all(), nullptr);
  const uint8_t hue = dialed.h;
  const uint8_t saturation = dialed.s;
  const uint8_t value = dialed.v;
